        ...
        */
        ...
        auto scan_deps = [&](this auto&& self, std::vector<std::string>& deps_paths) -> void {
            for (auto& path : deps_paths) {
                ...
                else if (path.starts_with("https://github.com/"sv)) {
                    auto out_path = std::format("cman.lib/{}"sv, fs::path(path).stem().string());
                    if (fs::exists(out_path)) continue;

                    std::println("\033[96m[cman] \033[36mCloning github repository: {}\033[0m"sv, path);
                    std::system(std::format("git clone \"{}\" cman.lib/{}"sv, path, out_path).data());
                }
